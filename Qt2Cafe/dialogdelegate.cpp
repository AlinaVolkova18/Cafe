                 ...
                 const QStyleOptionViewItem &, const QModelIndex &index)
{
    if (event->type() == QEvent::MouseButtonDblClick)
    {
        ...
        QString Type = model->data(model->index(index.row(),3,index)).toString();
        QString Time = model->data(model->index(index.row(),4,index)).toString();

        editDialog->SetUpDialog(ID, Name, Address, Type, Time);
        ...
        if (editDialog->exec() == QDialog::Accepted)
        {
            ...
            model->setData(model->index(index.row(),3,index),editDialog->type());
